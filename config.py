...
# -- Mac 快捷指令 OCR 配置 --
MAC_SHORTCUTS = "ocr-file"    # Mac 快捷指令名称，用于调用系统 OCR
                              # 如果此值不为空，则使用 Mac 快捷指令进行 OCR，而不使用 PaddleOCR
                              # 设置为空字符串 "" 或 None 则使用 PaddleOCR
                              # 用法：shortcuts run <快捷指令名称> -i <图片路径>
